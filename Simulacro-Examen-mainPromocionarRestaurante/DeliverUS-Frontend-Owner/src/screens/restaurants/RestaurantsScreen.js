...
export default function RestaurantsScreen ({ navigation, route }) {
  ...
  const renderRestaurant = ({ item }) => {
    return (
      ...
      >
        ...
        <TextSemiBold>This restaurant is <TextSemiBold textStyle={{ color: GlobalStyles.brandGreen }}>{item.status}</TextSemiBold></TextSemiBold>
        ...
        {item.status !== 'closed' && item.status !== 'temporaly closed' &&
        <Pressable
            onPress={() => { cambioStatus(item.id) }}
            style={({ pressed }) => [
              {
                backgroundColor: pressed
  ...
  const cambioStatus = async (id) => {
    try {
      ...
      showMessage({
        message: `There was an error while change status's restaurants. ${err} `,
        type: 'error',
