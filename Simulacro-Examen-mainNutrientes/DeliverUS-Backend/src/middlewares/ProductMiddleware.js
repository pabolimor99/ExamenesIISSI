...
const checkProductCalories = async (req, res, next) => {
  try {
    const calories = (req.body.proteinas * 4) + (req.body.grasas * 9) + (req.body.carbohidratos * 4)
    if (calories > 1000) {
      return res.status(409).send('Muchas calorias.')
    } else {
      return next()
    }
  } catch (err) {
...
export { checkProductOwnership, checkProductCalories, checkProductRestaurantOwnership, checkProductHasNotBeenOrdered }
