...
import { getAll, promot, remove } from '../../api/RestaurantEndpoints'
...
export default function RestaurantsScreen ({ navigation, route }) {
  ...
  const renderRestaurant = ({ item }) => {
    return (
      ...
      >
        ...
        {item.promoted &&
        <TextSemiBold>Promoted: <TextSemiBold textStyle={{ color: GlobalStyles.brandPrimary }}>PROMOCIONADO EN DESCUENTO EN UN {item.descuento}%</TextSemiBold></TextSemiBold>}
        ...
        <Pressable
            onPress={() => { promocionarRestaurante(item) }}
            style={({ pressed }) => [
              {
                backgroundColor: pressed
                  ? GlobalStyles.brandBlueTap
          ...
          <View style={[{ flex: 1, flexDirection: 'row', justifyContent: 'center' }]}>
            ...
            <TextRegular textStyle={styles.text}>
              {(item.promoted &&
              <TextSemiBold>Demote</TextSemiBold>) || (!item.promoted && <TextSemiBold>Promote</TextSemiBold>)}
            </TextRegular>
          </View>
        </Pressable>
        </View>
  ...
  const promocionarRestaurante = async (restaurant) => {
    try {
      await promot(restaurant.id)
