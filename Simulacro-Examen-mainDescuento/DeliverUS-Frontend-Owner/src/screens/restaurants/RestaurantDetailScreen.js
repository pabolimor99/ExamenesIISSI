...
import { remove, promtProd } from '../../api/ProductEndpoints'
...
export default function RestaurantDetailScreen ({ navigation, route }) {
  ...
  const renderProduct = ({ item }) => {
    return (
      ...
      >
        ...
        {item.promotedProduct && item.availability &&
        <TextRegular textStyle={styles.priceNew}>PROMOCION EL NUEVO PRECIO ES: {item.priceNew}€</TextRegular>}
  ...
  const promotProduct = async (producto) => {
    try {
      await promtProd(producto.id)
      await fetchRestaurantDetail()
    } catch (err) {
      console.log(err)
      setProductToBeDeleted(null)
      showMessage({
        message: `Product ${producto.name} could not be removed.`,
        type: 'error',
        style: GlobalStyles.flashStyle,
        titleStyle: GlobalStyles.flashTextStyle
      })
