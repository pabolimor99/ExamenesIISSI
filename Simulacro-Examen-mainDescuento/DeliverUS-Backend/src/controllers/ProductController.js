...
const promot = async function (req, res) {
  try {
    ...
    if (restaurant.promoted) {
      if (producto.promotedProduct) {
        producto.promotedProduct = false
      } else {
        producto.priceNew = producto.price - (restaurant.descuento * producto.price) / 100
        producto.promotedProduct = true
      }
    }
    res.json(await producto.save())
  } catch (err) {
    res.status(500).send(err)
  }
}
