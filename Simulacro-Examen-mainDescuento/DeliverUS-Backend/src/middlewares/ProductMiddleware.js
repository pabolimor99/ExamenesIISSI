...
const checkRestaurantDescuento = async (req, res, next) => {
  try {
    ...
    const restaurant = await Restaurant.findByPk(producto.restaurantId)
    if (restaurant.promoted && restaurant.descuento > 0) {
      return next()
    } else {
      return res.status(409).send('NOnono Wololo')
    }
  } catch (err) {
    return res.status(500).send(err)
  }
}
const checkCreateDescuento = async (req, res, next) => {
  try {
    const producto = await Product.findByPk(req.params.productId)
    const restaurant = await Restaurant.findByPk(producto.restaurantId)
    if (restaurant.promoted && restaurant.descuento > 0 && producto.promotedProduct) {
      return next()
    } else {
      return res.status(409).send('NOnono Wololo')
    }
...
export { checkProductOwnership, checkProductRestaurantOwnership, checkProductHasNotBeenOrdered, checkRestaurantDescuento, checkCreateDescuento }
